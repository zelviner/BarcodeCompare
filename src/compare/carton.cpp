...
bool Carton::compare(QString &error, bool &is_end) {
    ...
    int card_start_check_num = carton_info_->card_start_check_num - 1;
    int card_end_check_num   = carton_info_->card_end_check_num;
    ...
    QString target_iccid = carton_info_->target_iccid.mid(card_start_check_num, card_end_check_num - card_start_check_num);
    ...
    if (interval % carton_info_->card_count == 0) {
        correct_start_iccid_ = high_precision.add(target_iccid, carton_info_->card_count);
        int temp             = carton_info_->card_count * 2 - 1;
        correct_end_iccid_   = high_precision.add(target_iccid, temp);
    } else {
        correct_start_iccid_ = high_precision.add(target_iccid, 1);
        correct_end_iccid_   = high_precision.add(target_iccid, carton_info_->card_count);
    }
